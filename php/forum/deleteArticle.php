<?php
  ...
  try {
    ...
    if ((int) $article["mem_id"] !== (int) $memberId) {
        echo json_encode(["success" => false, "message" => "您沒有權限刪除這篇文章"], JSON_UNESCAPED_UNICODE);
        exit();
    }
    ...
    if($stmt2->rowCount() === 0){
      echo json_encode(["success" => false, "message" => "這篇文章已經是下架狀態，無法重複刪除"], JSON_UNESCAPED_UNICODE);
      exit();
    }
