...
const bottomBanner = document.querySelector(".battle-bottom-Banner"); //底部瀏覽過資訊後，建立邀約的banner圖
...
function renderBattleCards(battles) {
  ...
  //每次render前，都重新取得一次目前平台上可顯示的邀約有哪些；當陣列長度大於0時，即代表目前平台上並非完全沒有進行中的邀約
  const hasAvailableBattles = getAvailableBattles().length > 0;


  // 當完全沒有任何進行中的邀約時，顯示
  if (!hasAvailableBattles) {
    ...
    moreBtn.style.display = "none"; //完全沒資料時，不顯示「載入更多」按鈕
    bottomBanner.style.display = "none"; //底部的建立邀約banner也隱藏

    return;
  }

  // 當availableBattles中還有資料，而battles === 0時，代表平台上目前還有進行中的邀約，只是篩選後沒有符合的結果，顯示以下畫面
  if (battles.length === 0) {

    battleCardList.innerHTML = `
      <div class="battle-empty battle-empty-filter">
        <h3>目前沒有符合條件的對戰邀約</h3>

        <p>
          試著調整地區、日期或對戰條件，看看其他正在等待挑戰的邀約。
        </p>
      </div>
    `;
    ...
    // 篩選無結果時是否顯示 banner，可自行決定
    bottomBanner.style.display = "";
  ...
  const visibleBattles =
    currentBattleList.slice(0, visibleCardCount);

  battleCardList.innerHTML =
    visibleBattles.map(createBattleCard).join("");
